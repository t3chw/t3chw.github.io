...
**What it found.** Churn risk falls by a factor of 212 over the first 90 days and then goes flat for three years — so the first 90 days is where a *timed* intervention has something to aim at, while the larger absolute loss that follows has no window in it and needs an always-on mechanism instead. A third of everything the weakest segment will ever lose, it loses in those first 90 days. And value tracks headcount far more closely than the usual whale story assumes: the heaviest 12.1% of reviewers account for 19.6% of all expected retained time — 1.63× over-represented, against a structural ceiling of 1.91×.

This first part is about none of that. It is about the work that happens **before** any model: deciding what quantity is actually estimable from the data in hand, and refusing to estimate the one that isn't.

That sounds like throat-clearing. It isn't. Three decisions made here — where the clock starts, what counts as churn, and what to do with the 8.5% of people whose data is structurally strange — determine every number in Parts 2 through 5. Get them wrong and the rest of the work is precise and useless.
...
At first glance that is a regression problem. It isn't, and the reason is hidden in the shape of the data rather than in the modelling.
...
**We do not see an activity history.** We see two timestamps: the review, and the last session. There is no session log, so a user who quit for eight months and came back looks identical to one who played steadily throughout. Every design decision below is constrained by that.

**One field is a trap.** `playtime_forever` is measured at collection, so it contains behaviour that happened *after* the review. Its raw correlation with the outcome is **0.309** — it would have looked like a wonderfully predictive feature and would have been leakage. Only `playtime_at_review`, measured at the clock's origin, is used anywhere in this project.
...
- **Censored:** if they played within 14 days of collection, we only know they survived to the collection date.

Which gives 267,903 people, **202,795 observed churn events (75.7%)** and **65,108 censored (24.3%)**.
...
Not account lifetime. Every number in this series should be read with that scope attached. I put this in the second section rather than a limitations footnote because it defines what is being measured, not what is being apologised for.
...
The 14-day threshold is inherited from the data pipeline, which makes it a judgement call, which makes it something to test rather than defend. Part 5 re-runs the headline numbers at 7, 21 and 30 days. The levels move — a 7-day rule labels more people churned and shortens every estimate, as it must. The ordering of segments does not move at all, and the gap between the weakest and strongest stays between 3.80× and 3.96× across the entire range.

## 6. Censoring is not random, and that is the whole point

If you ignore censoring and average the observed durations, you get a number that means nothing, because "still going" observations are treated as if they had ended.

Worse, the censoring here is **administrative**: a person is censored because the study stopped, and the study stopped at the same moment for everyone. So how long you were observable depends entirely on when you wrote your review.
...
**9.1%** of the 2018 cohort is censored. **48.2%** of the 2026 cohort is. Any method that treats duration as a complete observation will conclude that VRChat's newer users are dramatically stickier. They are not; they are younger.

This same fact returns in Part 2 as a much subtler problem, where it quietly threatens one of the most interesting-looking findings in the dataset.
...
That group is not homogeneous, and the pipeline throws away the detail that would show it: durations are clipped at zero, so the *size* of the pre-review gap is discarded. At day resolution, 17,552 of them last played on a strictly earlier date and 5,245 last played on the review date itself. Someone who last played eight months before writing and someone who stopped the same morning are both recorded as `t = 0`, and they are plainly not the same person. Recovering that gap would be the first thing I would change about the pipeline. They are not fast churners. They are people who came back to write a review after they had already left, which is a different behaviour with a different meaning.

Three consequences followed.

**It cannot be modelled as a small duration.** Continuous survival distributions cannot put probability mass on a single point, and the Weibull log-hazard diverges at `t = 0` for shape below 1. Adding an epsilon would have been a fudge that hid a real structure. So the population is modelled as a **two-part (hurdle) process**:
...
Models 1–3 fit the second part only, on the 245,102 people with `t > 0`, and say so. Model 4 puts the halves back together. [Part 3](/posts/vrchat-retention-3-when-simple-models-fail/) shows the decomposition reproducing the full observed curve with a **mean absolute error of 0.03 percentage points** across horizons from 7 days to 7 years, worst case 0.08 pp — which is the empirical licence for splitting it this way.
...
**Dropping them is a real bug with a measurable size.** An earlier version of the pipeline excluded these rows as corrupt. Excluding them moves the Kaplan–Meier median from **962 days to 1,106 days** — a 15% overstatement, produced entirely by deleting the shortest-lived people in the sample.
...
One thing I cannot do is put a number on the selection. Steam does not publish how many accounts have played VRChat, so there is no denominator for "267,903 reviewers out of how many players?" — and review rates on Steam typically run in the low single-digit percent, which would make this a study of a small and unusual slice. The direction is also ambiguous: reviewers are plausibly both more engaged *and* more disgruntled than the base population, so it does not cleanly bias one way. What I can say is that every number here describes reviewers, and that the gap between reviewers and players is unmeasured rather than small.
...
|---|---|
| Separating the 8.5% already-gone | The weakest segment's value is overstated by **72%**, and the spread between weakest and strongest compresses from 3.9× to 2.3× — so a value-based targeting strategy points at the wrong users. Part 5 shows this happening to me |
...
**An already-departed reviewer is 1.89× as likely to leave a negative review** — and that widens to **2.36×** once you compare within playtime bands, so it is not a playtime artefact. They are 8.5% of reviewers and write **14.9% of all negative reviews**.
...
*Everything in this series is fit in NumPyro (Python 3.12 / JAX) in float64, with convergence gates on every model, PSIS-LOO for model comparison, and credible intervals on every published quantity. Data: public Steam review corpus for app 438100, collected 2026-08-28.*
