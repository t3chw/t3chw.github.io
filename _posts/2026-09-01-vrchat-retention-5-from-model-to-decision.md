...
> **The audit, in numbers.** 192 automated checks, all of which I re-ran while writing this series. 30/30 likelihood checks: eight are direct comparisons against SciPy, at ordinary values *and* deep in the censored tail, agreeing to machine epsilon; the rest are internal identities, of which the loosest is a numerical-quadrature check agreeing to 2.8e−5. 30/30 estimator checks against independent implementations — Kaplan–Meier against `lifelines` to about 1e-13, RMST to about 2e-14 (both are worst-case relative differences, and both sit a hair above the round tolerance I used to quote). 132/132 published-number checks against the saved posteriors. And a clean 23-step re-run from an empty output directory, which I *did* execute for this write-up: 21/21 tables and every posterior byte-identical, because the MCMC is seeded.
>
...
**What it supports:** cross-cohort error is **3.43 percentage points** of mean error in S(90) averaged over segments — per segment it runs **0.44 pp** (`1000h+`) to **8.60 pp** (`<1h`) — against **0.30 pp** (sd 0.13 over 200 seeds) for a random hold-out *within* cohorts. *(This read "1.8–4.4 pp" until 2026-09-05. That range is the **placebo split** result quoted two paragraphs down — 1.77 / 2.80 / 4.39 — not the cross-cohort error, so the same three numbers were appearing twice under two different labels.)* So **every elpd in this series understates the error you would get on a genuinely different cohort** by roughly an order of magnitude — and a different cohort is the only thing a retention team ever predicts. The ranking does not change, and stratification still helps.
...
> **And it turned up a data problem worth more than the validation.** `last_played` is a single timestamp, not a session history. For a reviewer we have only watched briefly, any lull in the 14 days before collection is recorded as terminal churn — while an identical lull by an old reviewer is invisible, because they came back and the timestamp moved forward. **So the churn rule's error rate depends on how long we have watched someone.** Measured S(90) by review cohort runs 87.0% (2020) down to **57.7%** for 2026, which has 149 days of median exposure. *(This sentence used to end "hold the observation window fixed and the collapse disappears entirely." **That is false — and it is the very claim the retraction two paragraphs below withdraws.** Capping the observation window changes S(90) by exactly 0.000 in every cohort, 2026 included, because S(90) reads only the first 90 days. Nor is the exposure story monotone: the 2018 cohort has the second-longest exposure in the dataset, 3,071 days, and S(90) of **71.5%** — below 2025's. 87.0% → 57.7% is a max-to-min range, not a trend. What survives is narrower: 2026, at 149 days of median exposure, has no room for a lull to be healed by a later return, and it cannot be corrected by any window manipulation — only excluded.)*
>
