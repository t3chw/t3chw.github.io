...
Adding five things you know about a user was worth less than getting the *timing* right. Much less. That is the headline, and it decides where the modelling effort on a retention problem is worth spending.

In technical terms: four models, in order, each fitted because the previous one failed in a way I could name. The most informative result in the whole project is not the winner. It is that **a model with five covariates lost to a model with none**, by a wide margin, because it had the wrong hazard shape. Flexibility in *time* mattered far more than flexibility in *people*. You need to know that before you decide where to spend your effort.
...
Start with the simplest model that could plausibly explain the data. Add complexity only when the data has rejected the assumption the simpler model was making. Never add a rung because the result would look more impressive.
...
One parameter for the entire platform: risk on day 1 equals risk on day 1,000.

Part 2's headline 212× is an all-users figure, driven by the day-zero atom. On the `t > 0` population these models are actually fitted on, the hazard still falls **46-fold** between the first day and day 90. Either way a constant rate is dead on arrival. I fitted this model precisely because it should lose: a baseline you expect to lose is the only way to know whether later complexity buys anything.

**The prior.** `log λ ~ Normal(−7, 1.5)`, centred on a median lifetime of about 760 days. That is the right order of magnitude, and it is deliberately *not* the 962 days the Kaplan–Meier curve shows, so the data is not used twice. Before fitting, I checked what the prior alone implies: it puts 95% of its mass on median lifetimes between 41 and 13,952 days, with the observed 962 comfortably inside that range.

**Result.** Constant hazard 0.076% per day, implied median 912 days.

The fair comparison is against the Kaplan–Meier median of the population M1 was actually fitted on. That is the 245,102 people with `t > 0`, whose median is **1,106 days**, not the 962 of the full sample. So M1 is 194 days short, not 50. It converged cleanly (R̂ 1.005, ESS 1,416, zero divergences), and its survival curve misses at both ends, exactly as predicted.

## 3. Model 2 — three smooth shapes, so "which shape?" becomes an empirical question

Weibull, log-normal and log-logistic all cost two parameters, and each allows a different hazard shape. Weibull's can only move one way, up or down, never both. The other two *can* rise and then fall.

Fitting all three turns a modelling preference into a measurement. And one result is worth noting: the fitted log-logistic shape came out at 0.94, below 1, so its hazard falls the whole way too. Given the freedom to bend, it declined to.

**Priors in business units.** For Weibull and log-logistic: `log(scale) ~ Normal(7, 1.5)`, where e⁷ is 1,097 days, and `log(shape) ~ Normal(0, 0.5)`, centred on 1.

For the Weibull, shape 1 *is* the exponential. So centring there means the prior assumes no decay at all. The posterior lands at 0.813, which makes the falling hazard visibly the data speaking rather than the prior. (The log-normal is set up differently — `mu ~ Normal(7, 2)`, `s ~ HalfNormal(2)` — because it has no shape parameter of the same kind.)
...
That error pattern is the signature of a shape mismatch. The families are not badly estimated. They are being asked to make a shape they cannot make, so they compromise — and the compromise shows up as a systematic pattern of over- and under-shooting rather than as noise.

> ### A correction, made while writing this series
>
> Models 1 and 2 are fitted on the 245,102 people with `t > 0` (Part 1). But their published survival-curve error was measured against the Kaplan–Meier curve of **all 267,903**. That curve sits about 8 points lower early on, because it contains the day-zero atom. So I was comparing a model of one population against the curve of a different one. Model 3's error was measured correctly, against the `t > 0` curve.
>
...
>
> The out-of-sample comparison below is unaffected. Every model's score is computed on the same fixed 25,000 people, drawn with the same seed from the same `t > 0` population. For M4 the score covers the **survival part only**; the day-zero part is left out, so all seven models are scored on exactly the same quantity.
{: .prompt-warning }
...
**The cut points are not guessed.** They are packed close together where Part 2 measured the hazard moving fastest — 1, 3, 7, 14, 30, 60, 90, 180 days. Then half-yearly to two years (365, 547, 730), and yearly after that (1095, 1460, 1825, 2190, 2555), closing with 3000 and 4000.

The last three cuts sit beyond the bands Part 2 measured. They exist to give the thin tail somewhere to go, not because the data pointed at them.
...
**Mean absolute error: 0.22 percentage points on a daily grid.** (0.03 pp if you only look at the model's own cut points, which is the flattering number and the one I originally published.)
...
They agree to **0.25%** at every interval. With 245,102 people the thinnest interval still holds 748 events, so the data pins each hazard down on its own and there is essentially nothing for a smoothing prior to do.

I keep the independent version for speed — **2.5 s versus 94.1 s, ESS 11,009 versus 682**. The random walk would be the correct choice on a thinner dataset.

> ### A correction I had to make to my own diagnosis
>
> An earlier version reported that the random-walk prior was **weakly identified**, citing R̂ 1.06 and ESS 58, and gave a structural explanation for it. That measurement was taken in float32. Under float64, the same model with identical settings gives R̂ 1.004, ESS 682. The bad mixing was arithmetic precision, not statistics. [Part 4 §6](/posts/vrchat-retention-4-beyond-the-average-user/) has the second and worse instance, with the controlled re-run.
{: .prompt-warning }

### The engineering that makes it fast

For a piecewise-exponential model without covariates, the likelihood collapses **exactly** into two summaries per interval: how many people left in it, and how much time was lived inside it. That is 36 numbers instead of 245,102 rows, with no approximation.

Written the obvious way, the model multiplies a 245k × 18 matrix inside every single gradient step. Written this way, it samples in **2.5 seconds**.

Every downstream result rests on that shortcut, so I tested it rather than trusting it: it reproduces the row-by-row likelihood to **1e-16**. The per-person values needed for model comparison are rebuilt afterwards from the draws.

> Two silent memory bugs are worth recording, because both produced *correct numbers* and would never have shown up in a result. `az.from_numpyro(..., log_likelihood=True)` saves one value per person per draw. At 245,102 × 4,000 that is a **7.3 GB posterior for a one-parameter model**. And in Model 4 the AFT scale was registered with `numpyro.deterministic`. It has one value per person, so every draw stored a 245,102-long vector: **3.9 GB for a 13-parameter model**. The rule that follows: never save a derived quantity whose first dimension is the sample size.
{: .prompt-info }
...
For the second part I chose log-logistic over log-normal — that is the comparison the code actually makes. It won on predictive score between those two (−151,791 against −154,951), and it has the heavier tail. The tail matters here because **26.6% of this sample is censored** and therefore sits in it. (26.6%, not the 24.3% of the full population: part 2 is fitted on `t > 0`, and everyone in the day-zero atom is an event.)

> The obvious cross-question: Weibull beat both on predictive score, and Weibull is also an AFT family, so why not use it? Because a Weibull hazard can only move one way, and Part 2's entire finding is that this hazard does not. The fitted log-logistic shape is 1.014, so it can bend. The deeper answer is that it did not matter. M4 lost to M3 whichever family went inside it, so the family choice was never what decided the result.
{: .prompt-info }

All continuous covariates are put on a common scale, so one prior width is meaningful for all of them. The AFT coefficients get `β ~ Normal(0, 0.5)`, which allows lifetime multipliers between **0.38× and 2.66×** per standard deviation across 95% of the prior. The day-zero coefficients get the wider `Normal(0, 1)`, because that scale needs more room. Both keep the estimates in a sensible range without deciding the answer.
...
If I had started with a covariate model — the instinct most of us have — I would have spent the effort on the axis that mattered less. And the fit statistics would have looked respectable enough that I might never have noticed.
...
Every comparison was already decisive and is now overwhelmingly so. No conclusion changes. But the naive form is the kind of error that *would* change a conclusion in a closer race, so it is worth fixing in a race where it does not.

## 7. Where that leaves the model

M3 has the time structure and no way to tell people apart. M4 can tell people apart and has the wrong time structure. And Part 2 measured a 3.9× spread between segments, so ignoring people is not an option either.
...
**Model choice has an infrastructure cost, and it is not the one you expect.** The piecewise-exponential likelihood collapses exactly into 36 numbers, so the fit no longer depends on how many rows there are: 2.5 seconds on 245,102 people. The same model written the obvious way multiplies a 245k × 18 matrix inside every gradient step. That is the difference between a model you can re-run on every data refresh and one you run once and never touch. I verified the shortcut to 1e-16 precisely because so much rests on it.

**Never save a derived quantity whose first dimension is the sample size.** Two silent memory bugs in this project produced perfectly correct numbers while writing multi-gigabyte files — 7.3 GB for a one-parameter model. Nothing in any result would ever have shown it.

**And the wider point:** every rung of this ladder is a model I expected to lose, fitted so that the next one had something to beat. The failures are the deliverable. Jumping straight to the hierarchical model produces identical final numbers and no way to defend them.
